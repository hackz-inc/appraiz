...
export default function ScoringPage() {
  ...
  const handleSubmit = async () => {
    ...
    const allScoresFilled = itemsList.every(item =>
      scoreInputs[item.id] !== undefined
    )
        ...
        <Card variant="elevated" className="mb-6">
          ...
          <div className="space-y-6">
            {itemsList.map((item) => {
              ...
              return (
                <div key={item.id} className="border-t border-black-lighten3 pt-4 first:border-t-0 first:pt-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Array.from({ length: item.max_score + 1 }, (_, i) => i).map(
                      (score) => (
                        <button
                          type="button"
                          key={score}
