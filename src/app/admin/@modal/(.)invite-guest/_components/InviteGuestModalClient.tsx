...
type Props = {
	hackathonId: string;
	guestList: GuestWithInviteStatus[];
};

export const InviteGuestModalClient = ({ hackathonId, guestList }: Props) => {
	...
	const loadGuests = useCallback(async () => {
		...
		try {
			...
			const invitedIds = new Set(
				guestList.filter((g) => g.isInvited).map((g) => g.id),
			);
	...
	return (
		<Modal isOpen={true} onClose={handleClose} title="ゲストを招待" size="lg">
