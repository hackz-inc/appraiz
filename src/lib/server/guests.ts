...
export const getGuestsWithInviteStatus = cache(async (hackathonId: string) => {
  ...
  const invitedGuestIds = new Set(invitedGuests?.map((g: any) => g.guest_id) || [])

  const guestsWithStatus: GuestWithInviteStatus[] = allGuests.map((guest: Guest) => ({
    ...guest,
